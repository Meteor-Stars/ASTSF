...
def MAPE_elect(pred, true,args=None):
    fund_metric = {}
    me_name = ['consumption']
    for k in range(pred.shape[-1]):
        fund_metric[me_name[k]] = calculate_wmape(np.expand_dims(pred[:, :, k], axis=-1),
...
class MAPE_Fund():
    ...
    def cal_fund_val(self,pred, true):
        fund_metric={}
        rse = RSE(pred, true)
        rse=np.round(rse*100, 2)
        pmae = PMAE(pred, true, self.args)
        pmae = np.round(pmae, 2)
        if 'elect' in self.args.data_type:
            me_name = ['consumption']
            for k in range(pred.shape[-1]):
                fund_metric[me_name[k]] = calculate_wmape(np.expand_dims(pred[:, :, k], axis=-1),
                                                          np.expand_dims(true[:, :, k], axis=-1)) * 100
            fund_metric['sum'] = fund_metric['consumption']
            fund_metric['rse'] = float(rse)
            fund_metric['pmae'] = float(pmae)
            return fund_metric
        elif 'Fund' in self.args.data_type:
            me_name=['apply','redeem']
            for k in range(pred.shape[-1]):
...
def metric(pred, true,args=None):
    ...
    # mspe = MSPE(pred, true)
    rse = RSE(pred, true)
    corr = np.mean(CORR(pred, true))
    pmae=PMAE(pred,true,args)
    if 'elect' in args.data_type:
        mape_fund=MAPE_elect(pred, true, args)
    elif 'Fund' in args.data_type:
        mape_fund = MAPE_fund(pred, true, args)

    # mape_fund['mae']=mae
    # mape_fund['mse']=mse
    # mape_fund['rmse']=rmse
    mape_fund['rse']=rse*100
    mape_fund['corr']=corr*100
    mape_fund['pmae']=pmae
    for k,v in mape_fund.items():

        mape_fund[k]=np.float(np.round(v,2))

    print('******',pred.shape,true.shape)
    return pmae, rse, corr,mape_fund
