...
class TimeFeatureEmbedding_new(nn.Module):
    ...
    def forward(self, x, scale=1):
        if (scale, x.shape[0], x.shape[1]) not in self.concats:
            # print('**')
            concat_tensor = torch.tensor([[[1/scale-0.5]]]).to('cuda:5').repeat(x.shape[0],x.shape[1],1)
            self.concats[(scale, x.shape[0], x.shape[1])] = concat_tensor
        else:

            concat_tensor = self.concats[(scale, x.shape[0], x.shape[1])]
        # print(x.device,concat_tensor.device)
        # time.sleep(500)
...
class DataEmbedding(nn.Module):
    def __init__(self, c_in, d_model, embed_type='fixed', freq='h', dropout=0.1,args=None):
        ...
        # self.cov_embedding = TokenEmbedding(c_in=6, d_model=d_model)
        ...
        self.id_embd=nn.Embedding(200,d_model)
    def forward(self, x, x_mark):

        x = self.value_embedding(x) + self.position_embedding(
            x) + self.temporal_embedding(x_mark)  # + id_emb
...
class DataEmbedding_mine(nn.Module):
    ...
    def forward(self, x, x_mark, scale, first_scale, label_len):
        ...
        # print(x.shape) #torch.Size([32, 4, 4])
